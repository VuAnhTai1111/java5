...
<hr>
<form action="/mau-sac/create" method="get">
...
<table class="table table-striped">
    ...
    <c:forEach items="${data.content}" var="ms">
        <tr>
            ...
            <td><a href="/mau-sac/edit/${ms.id}">Update</a></td>
            <td><a href="/mau-sac/delete/${ms.id}">Delete</a></td>
        </tr>
    </c:forEach>
<%--    <tr>--%>
<%--        <td>1</td>--%>
...
<nav aria-label="Page navigation example" style="margin-left: 30%">
    <ul class="pagination">
        <li class="page-item" aria-disabled="true">
            <c:if test="${ page <= 0}">
                <a class="page-link" href="/mau-sac/index?keyword=${keyword}&&page=${data.number - 1}" aria-disabled="true">Previous</a>
            </c:if>
            <c:if test="${ page != 1}">
                <a class="page-link" href="/mau-sac/index?keyword=${keyword}&&page=${data.number - 1}">Previous</a>
            </c:if>
        ...
        <c:forEach begin="0" end="${ data.totalPages }" var="page">
            <c:if test="${ page == 0 || page == data.totalPages -1 || ( page >= data.number - 1 && page <= data.number + 1 && page < data.totalPages) }">
                ...
                <li class="page-item">
                    <a class="page-link"
                       href="/mau-sac/index?keyword=${keyword}&&page=${page}">
                            ${ page + 1}
                    </a>
                </li>
            </c:if>
        ...
        <li class="page-item" ><a class="page-link" href="/mau-sac/index?keyword=${keyword}&&page=${data.number + 1}">Next</a></li>
    </ul>
</nav>
</body>
</html>
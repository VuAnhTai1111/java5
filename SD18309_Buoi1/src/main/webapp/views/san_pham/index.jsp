...
<hr>
<form action="/san-pham/create" method="get">
...
<nav aria-label="Page navigation example" style="margin-left: 30%">
    <ul class="pagination">
        <li class="page-item" aria-disabled="true">
            <c:if test="${ page <= 0}">
                <a class="page-link" href="/san-pham/index?keyword=${keyword}&&page=${data.number - 1}" aria-disabled="true">Previous</a>
            </c:if>
            <c:if test="${ page != 1}">
                <a class="page-link" href="/san-pham/index?keyword=${keyword}&&page=${data.number - 1}">Previous</a>
            </c:if>
        ...
        <c:forEach begin="0" end="${ data.totalPages }" var="page">
            <c:if test="${ page == 0 || page == data.totalPages -1 || ( page >= data.number - 1 && page <= data.number + 1 && page < data.totalPages) }">
                ...
                <li class="page-item">
                    <a class="page-link"
                       href="/san-pham/index?keyword=${keyword}&&page=${page}">
                            ${ page + 1}
                    </a>
                </li>
            </c:if>
        ...
        <li class="page-item" ><a class="page-link" href="/san-pham/index?keyword=${keyword}&&page=${data.number + 1}">Next</a></li>
    </ul>
</nav>
